...
const toHumanAmount = ({ originAmount, decimals }: { originAmount: any; decimals: number }) => {
  try {
    if (!originAmount) {
      return 0;
    }
...
export const formatPrice = ({ price, convertToNumber = false }: { price: any; convertToNumber?: boolean }) => {
  const pDecimals = 9;
  const originalAmount: any = toOriginalAmount({ humanAmount: price, decimals: pDecimals, round: true }) || 0;
...
export default { toOriginalAmount, formatPrice, amountVer2, toHumanAmount, toNumber };
