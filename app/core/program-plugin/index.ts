...
const log = createLogger("incognito:decoder");
...
export class ProgramPluginManager {
  ...
  render = async (
    ...
    const decodeInstructionFunc = async (idx: number, instruction: TransactionInstruction): Promise<Markdown> => {
      ...
      try {
        ...
        log("An error occurred when decoding instruction for program [%s] %o", programId.toBase58(), error);
    ...
    return Promise.all(transaction.instructions.map((instruction, index) => decodeInstructionFunc(index, instruction)));
  };
